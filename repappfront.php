...
//								WHERE APPLICANTNO=$applicantno LIMIT 2") or die(mysql_error());
	...
	$qrydoclist = mysql_query("SELECT cd.DOCUMENT,a.VALIDITY
								FROM applicantdocs a
								LEFT JOIN crewdocuments cd ON cd.DOCCODE=a.DOCCODE
								WHERE cd.TYPE='D' and a.APPLICANTNO=$applicantno") or die(mysql_error());

	$qryliclist = mysql_query("SELECT cd.DOCUMENT,a.VALIDITY,a.HIGHERLIC
								FROM applicantdocs a
								LEFT JOIN crewdocuments cd ON cd.DOCCODE=a.DOCCODE
								WHERE cd.TYPE='L' and a.APPLICANTNO=$applicantno") or die(mysql_error());
...
//	$qryreferencelist = mysql_query("SELECT * FROM applicantreference WHERE APPLICANTNO=$applicantno LIMIT 2") or die(mysql_error());
	...
	$qrypersonallist = mysql_query("SELECT CONCAT(FNAME,', ',GNAME,' ' ,MNAME) AS NAME, 
									...
									r1.RANK AS CHOICE1,r2.RANK AS CHOICE2,DATEAPPLIED,IF(c.RECOMMENDEDBY IS NULL,a.RECOMMENDEDBY,c.RECOMMENDEDBY) AS RECOMMENDEDBY,
									QUESTION1A,QUESTION1B,QUESTION2A,QUESTION2B,QUESTION3A,QUESTION3B,QUESTION4A,QUESTION4B,
...
";			
					while ($rowdoclist=mysql_fetch_array($qrydoclist))
					{
						...
						if ($rowdoclist["VALIDITY"] != "")
							$document2 = date('dMY',strtotime($rowdoclist["VALIDITY"]));
						else 
							$document2 = "";
echo "					
						<tr>
...
";			
					while ($rowliclist=mysql_fetch_array($qryliclist))
					{
						...
						if ($rowliclist["VALIDITY"] != "")
							$license2 = date('dMY',strtotime($rowliclist["VALIDITY"]));
						else 
							$license2 = "";
						
						if($rowliclist["HIGHERLIC"] == 1)
						{
							$license3 = $rowliclist["HIGHERLIC"];
		echo "						
							<tr>
								<td valign=\"top\">$license1&nbsp;<i><b>( with Higher License)</b></i></td>
