...
$localdir="scanned"; // local dumping
...
if(!empty($ccid))
	$addingccid="/CCID".$ccid;
	
switch ($actiontxt)
{
	case "renamefile":
		$getdoctype=$doccode[2];
		//ADDED BY CPA FOR CCID OF CREW
		if(!empty($ccid))
			$getdoctype=$getdoctype."/CCID".$ccid;
...
//			echo "<script>alert('$tst')</script>";
		...
		copy($localdir.'/'.$getplacefilename,$serverdir.'/'.$applicantno.'/'.$getdoctype.'/'.$getnewfile);
		unlink($localdir.'/'.$getplacefilename);
	break;
	case "dumpfile":
		
		$getnewfile=$applicantno."_".$docfile;
//		echo $serverdir.'/'.$applicantno.'/'.$doctypefolder.'/'.$docfile;
//		echo "$serverdir.'/'.$applicantno.'/'.$doctypefolder.'x'.$addingccid.'/'.$docfile";
		copy($serverdir.'/'.$applicantno.'/'.$doctypefolder.$addingccid.'/'.$docfile,$localdir.'/'.$getnewfile);
		unlink($serverdir.'/'.$applicantno.'/'.$doctypefolder.$addingccid.'/'.$docfile);
	break;
	case "deletefile":
		unlink($localdir.'/'.$getplacefilename);
	break;
...
if ($handle = opendir($localdir)) 
{
    	...
    	if ($file != "." && $file != "..") 
    	{
	        ...
	        if($file_parts['extension']=='pdf')
	        {
	        	...
	        	$qryname=mysql_query("SELECT CONCAT(FNAME,', ',GNAME) AS CREWNAME 
					FROM crew WHERE APPLICANTNO='$file'") or die(mysql_error());
	        	$rowname=mysql_fetch_array($qryname);
    			$crewname=$rowname["CREWNAME"];
	        	$files[] = $crewname."_".$file_parts['filename'];
	        }
    	}
    	$cnt++;
    }
	...
	if($files)
	{
	    ...
	    $filearrtemp="";
	    /*$getapplicantno="
	    	<span style=\"width:100%;height:30px;text-align:center;font-size:10pt;font-weight:bold;
	    		background:#8FBC8F;color:#704370;\">Crew&nbsp;&nbsp;&nbsp;
				<select name=\"applicantno\" style=\"width:200px;color:#704370;\"
	    ...
	    $getapplicantno="
	    	<div style=\"width:100%;height:90px;overflow:hidden;\">
				...
				&nbsp;<select name=\"applicantno\" style=\"width:250px;color:#704370;\"
					onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
					onchange=\"ccid.value='';resetdata();submit();\">\n
					<option value=\"\">-Select-</option>";
...
//	    		}
		...
		$getapplicantno .="
					...
					<br>&nbsp;&nbsp;&nbsp;CCID:&nbsp;&nbsp;<input type=\"text\" name=\"ccid\" size=\"8\" onblur=\"if(this.value!='$ccid'){submit();}\" onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\" style=\"font-size:0.8em;\" value=\"$ccid\">
				</div>
...
if(!empty($applicantno))
{
	...
	if ($handle = opendir($localdir)) 
	{
	    ...
	    if($filesappno)
	    {
	    	...
	    	if(empty($placefilename))
	    	{
	    		...
	    		$defaultimg="$localdir/$placefilename#toolbar=0&navpanes=0";
	    		$fileposition=0;
	    	}
	    	$getimg="";
	...
	{
		//ADDED BY CPA FOR CCID OF CREW
		if(!empty($ccid) && $type[$i]=="D")
			$addccid="/CCID".$ccid;
		else 
			$addccid="";
			
		$getdirtype=$serverdir.'/'.$applicantno.'/'.$type[$i].$addccid;
				    			...
				    			$getlistcert.= "
									<tr style=\"border-top: 1px solid gray;\" $mouseovereffect 
										...
										<td style=\"text-align:left;vertical-align:middle;cursor:pointer;\"
											onclick=\"doctypefolder.value='C';docfile.value='{$getvaluedump[1]}';document.getElementById('zoomfile').innerHTML='{$getvaluedump[0]}';
				    		...
				    		if($docname)
				    		{
				    			...
				    			$getlistdoc.= "
									<tr style=\"border-top: 1px solid gray;\" $mouseovereffect 
										ondblclick=\"\">
										<td style=\"text-align:left;vertical-align:middle;cursor:pointer;\"
											onclick=\"doctypefolder.value='D';docfile.value='{$getvaluedump[1]}';document.getElementById('zoomfile').innerHTML='{$getvaluedump[0]}';
												document.getElementById('dumpfile').src='$serverdir/$applicantno/D$addingccid/{$getvaluedump[1]}' 
												+ '#toolbar=0&navpanes=0';\">
											&nbsp;{$getvaluedump[0]}
										</td>
										<td style=\"text-align:right;vertical-align:middle;\">&nbsp;X</td>
				    		...
				    		if($docname)
				    		{
				    			...
				    			$getlistlic.= "
									<tr style=\"border-top: 1px solid gray;\" $mouseovereffect 
										ondblclick=\"\">
										<td style=\"text-align:left;vertical-align:middle;cursor:pointer;\"
											onclick=\"doctypefolder.value='L';docfile.value='{$getvaluedump[1]}';document.getElementById('zoomfile').innerHTML='{$getvaluedump[0]}';
...
//			document.getElementById('crewdetails').style.display='none';
		...
		with(document.filemanipulation)
		{
...
function createurl(x)
{
	if(x==1)
		...
		url = \"http://$getserveraddr/veritas/filemanipulationrequest.php?actionajax=\"; // The server-side script 
	
	var actionajax = document.filemanipulation.actionajax.value; 
		
	//fill URL
	if(document.filemanipulation.doctype)
		var doctype = '&doctype=' + document.filemanipulation.doctype.value;
	else
		var doctype = '&doctype=';
	var applicantno = '&embapplicantnohidden=' + document.filemanipulation.applicantno.value;
...
function prevnext(x)
{
	var prevnextfile = document.filemanipulation.filestorage.value.split(',');
	if(x==0)
	{
		var getposition = document.filemanipulation.fileposition.value*1 - 1;
	}
	else
	{
		var getposition = document.filemanipulation.fileposition.value*1 + 1;
	}
	document.getElementById('scannedfile').src='$localdir/' + prevnextfile[getposition] + '#toolbar=0&navpanes=0';
	document.filemanipulation.fileposition.value = getposition;
	var getpage=getposition+1;
	document.filemanipulation.placefilename.value=prevnextfile[getposition];
	document.filemanipulation.posfilename.value='(' + getpage + '/' + $cnt + ')';
	
	document.filemanipulation.btnprev.disabled=false;
	document.filemanipulation.btnnext.disabled=false;
	if(getpage==1)
		document.filemanipulation.btnprev.disabled=true;
	if(getpage==$cnt)
		document.filemanipulation.btnnext.disabled=true;
}
...
function viewpdf(x,loc) //loc - 1 for local, 2 for server
{
	var location;
	if(loc==1)
		location='$localdir/';
...
function chkrename()
{
	if(document.filemanipulation.doccodelist.value)
	{
		var chkfilearray = document.filemanipulation.filestoragedump.value.split(',');
		var getdoccode = document.filemanipulation.doccodelist.value.split('*');
		var rem;
		var gorename=1;
		...
		if(gorename==1)
		{
			document.filemanipulation.actiontxt.value='renamefile';
	//		alert('Ayos!');
			document.filemanipulation.submit();
		}
	}
	else
	{
...
function senddump()
{
	if(document.getElementById('dumpfile').src!='')
	{
		...
		{
			document.filemanipulation.actiontxt.value='dumpfile';
			document.filemanipulation.submit();
		}
	}
	else
...
function chkdelete()
{
	if(document.filemanipulation.placefilename.value)
	{
		if(confirm('Are you sure you want to delete this file?'))
		{
			document.filemanipulation.actiontxt.value='deletefile';
			document.filemanipulation.submit();
		}
	}
}
function chkdeleteimage() //for renamed images
{
	if(document.getElementById('dumpfile').src!='')
	{
		if(confirm('Are you sure you want to delete this file?'))
		{
			document.filemanipulation.actiontxt.value='deleteimage';
			document.filemanipulation.submit();
		}
	}
	else
...
<form name=\"filemanipulation\" method=\"POST\">\n

<span class=\"wintitle\">FILE MANIPULATION</span>
<div style=\"width:370px;height:600px;background:gray;float:left;\">";
	echo $getapplicantno;
	...
	<iframe id=\"scannedfile\" src=\"$defaultimg\" height=\"370\" width=\"300\"></iframe>
	<div id=\"imgrename\">
...
<div style=\"width:640px;height:600px;background:black;float:left;overflow:hidden;\">
	...
	<div id=\"dumpfilelist\" style=\"width:320px;height:550px;background:black;float:left;text-align:center;\">
		...
		<span style=\"font-weight:bold;color:yellow;width:300px;\">CERTIFICATES</span>
		...
		<span style=\"font-weight:bold;color:yellow;width:300px;\">DOCUMENTS $ccid</span>
		<div style=\"width:320px;height:150px;background:silver;overflow:auto;\">
			<table id=\"certimage\" class=\"listcol\" cellspacing=\"1\" cellpadding=\"0\">
				$getlistdoc
			</table>
		</div>
		<span style=\"font-weight:bold;color:yellow;width:300px;\">LICENSE</span>
		<div style=\"width:320px;height:150px;background:silver;overflow:auto;\">
			<table id=\"certimage\" class=\"listcol\" cellspacing=\"1\" cellpadding=\"0\">
				$getlistlic
			</table>
...
<input type=\"hidden\" name=\"dumpfileselect\">
<input type=\"hidden\" name=\"actionajax\">
<input type=\"hidden\" name=\"actiontxt\">
<script>
	document.filemanipulation.filestorage.value='$getimg';
	document.filemanipulation.filestoragedump.value='$getimgdump';
