...
if(isset($_POST['fleet_remarks']))
	$fleet_remarks = $_POST['fleet_remarks'];
...
switch ($actiontxt)
{
	case "save"	:
			...
			$fleet_remarks = mysql_real_escape_string($fleet_remarks);
		
			$qryupdate = mysql_query("UPDATE debriefinghdr SET FLEETBY='$employeeid', 
														FLEETDATE='$currentdate', 
														FLEETREMARKS='$fleet_remarks'
									WHERE CCID=$ccid") or die(mysql_error());
		
		break;
}

$qrygetremarks = mysql_query("SELECT FLEETREMARKS, rehire_assesment FROM debriefinghdr WHERE CCID=$ccid") or die(mysql_error());

$rowgetremarks = mysql_fetch_array($qrygetremarks);
$fleet_remarks = stripslashes($rowgetremarks["FLEETREMARKS"]);

if (!empty($fleet_remarks) && $departmentid != 5)  //DEPARTMENT ID = 5 (MIS GROUP) -- THEY CAN EDIT!!
{
...
<html>\n
<head>\n
<title>Fleet Debriefing Remarks</title>
<link rel=\"StyleSheet\" type=\"text/css\" href=\"veripro.css\" />
<script type='text/javascript' src='veripro.js'></script>

<body onload=\"\" style=\"overflow:hidden;background-color:White;\">\n

<form name=\"fleetdebriefing\" method=\"POST\">\n

<span class=\"wintitle\">FLEET DEBRIEFING - ARRIVING SEAMAN</span>
	...
	<div style=\"width:100%;height:220px;background-color:White;\">
		...
		<div style=\"width:50%;float:left;height:220px;background-color:Black;\">
			<span class=\"sectiontitle\">FLEET REMARKS</span>
			...
			<table style=\"width:90%;font-size:0.8em;\">
				...
				<tr>
					<td><textarea rows=\"8\" cols=\"55\" $readonly name=\"fleet_remarks\">$fleet_remarks</textarea></td>
				</tr>
				<tr>
					<td align=\"center\">
<!--
						<input type=\"button\" value=\"Save Remarks\"  $disabled
								style=\"border:1px solid Yellow;background-color:Green;color:White;font-weight:Bold;\"
								onclick=\"actiontxt.value='save';submit();\" />
								
										opener.document.debriefingstatus.applicantno.value='$applicantno';
...
-->								
						<input type=\"button\" value=\"Done\" $disabled
								style=\"border:1px solid Yellow;background-color:Green;color:White;font-weight:Bold;\"
								onclick=\"if(fleet_remarks.value != '') {if (confirm('Save changes?')) {
