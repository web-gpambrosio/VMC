...
if (isset($_POST['principalremarks']))
	$principalremarks = $_POST['principalremarks'];
...
switch ($actiontxt)
{
	case "approve"	:
			...
			$qryverify = mysql_query("SELECT APPLICANTNO FROM applicantstatus WHERE APPLICANTNO=$applicantno AND STATUS=1") or die(mysql_error());
			
			if (mysql_num_rows($qryverify) > 0)
			{
				$qryappstatusupdate = mysql_query("UPDATE applicantstatus SET PRINCIPALBY='$employeeid', PRINCIPALDATE='$currentdate', 
												PRINCIPALSTATUS=1, PRINCIPALREMARKS='$principalremarks'
												WHERE APPLICANTNO=$applicantno AND STATUS=1") or die(mysql_error());
			}
...
$qryforapproval = mysql_query("SELECT at.APPLICANTNO,CONCAT(FNAME,', ',GNAME,' ',MNAME) AS NAME,ax.PASSED,
							...
							WHERE at.APPROVEDDATE IS NOT NULL AND at.PRINCIPALDATE IS NOT NULL AND at.STATUS=1
        					$wherepart
        					$whererank
...
<span class=\"wintitle\">PRINCIPAL APPROVAL</span>
...
echo "
	...
	";
		...
		";
			...
			if ($applicantno != "")
			{
				$qrygetremarks = mysql_query("SELECT APPLICANTNO,ACCEPTBY,ACCEPTDATE,REMARKS AS FLEETREM, 
											APPROVEDBY,APPROVEDDATE,VMCREMARKS AS MGMTREM 
				...
				$mgmtapprovedby = $rowgetremarks["APPROVEDBY"];
				
				if ($rowgetremarks["APPROVEDDATE"] != "")
					$mgmtapproveddate = "[" . date("dMY",strtotime($rowgetremarks["APPROVEDDATE"])) . "]";
				else 
					$mgmtapproveddate = "";
					
				$mgmtremarks = $rowgetremarks["MGMTREM"];
			}
		...
		echo "
			...
			<span class=\"sectiontitle\">MANAGEMENT</span>
			<br />
			...
			<div style=\"width:100%;height:45%;\">
				<table style=\"width:100%;font-size:0.8em;font-weight:Bold;color:White;\">
					...
					<tr>
						<td>$mgmtapprovedby $mgmtapproveddate</td>
					</tr>
					<tr><td>&nbsp;</td></tr>
					<tr>
						<td $styletitle>Remarks:</td>
		...
		<div style=\"width:815px;height:140px;background-color:Black;overflow:hidden;margin: 0 0 0 10;\" >
			<table style=\"color:White;font-size:0.7em;font-weight:Bold;width:30%;float:left;\">
				<tr>
					<td>Remarks<br />
						<textarea rows=\"6\" cols=\"40\" name=\"principalremarks\">$principalremarks</textarea>
					</td>
				</tr>
			</table>
			...
			<table style=\"width:50%;height:60px;margin:15px;float:right;\" cellpadding=\"0\" cellspacing=\"0\">
				...
				<tr height=\"40px;\">
					<td style=\"width:50%;background-color:Green;\" >
						<input type=\"button\" value=\"APPROVED\" $disablebuttons style=\"border:1px solid White;width:100%;height:100%;cursor:pointer;font-size:1.3em;font-weight:Bold;color:White;background:inherit;\" 
										onclick=\"if(confirm('Approve Applicant?')){actiontxt.value='approve';submit();}\" />
					</td>
					<td style=\"width:50%;background-color:Red;\" >
						<input type=\"button\" value=\"DENIED\" $disablebuttons style=\"border:1px solid White;width:100%;height:100%;cursor:pointer;font-size:1.3em;font-weight:Bold;color:White;background:inherit;\" 
										onclick=\"if(confirm('Applicant Denied. Confirm?')){actiontxt.value='denied';submit();}\" />
					</td>
				</tr>
