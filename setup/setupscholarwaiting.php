...
$formname = "formscholarwaiting";
$formtitle = "SCHOLAR WAITING SETUP";
...
switch ($actiontxt)
{
	case "save"		:
			...
			$qryverify = mysql_query("SELECT IDNO FROM scholarwaiting WHERE FNAME='$fname' AND GNAME='$gname' AND MNAME='$mname'") or die(mysql_error());
		
			if(mysql_num_rows($qryverify) == 0)
			{		
				$qryscholarwaitingsave = mysql_query("INSERT INTO scholarwaiting(FNAME,GNAME,MNAME,DIVCODE,MADEBY,MADEDATE)
												VALUES('$fname','$gname','$mname','$divcode','$employeeid','$currentdate')
												") or die(mysql_error());
			}
			else 
			{
				$rowverify = mysql_fetch_array($qryverify);
				$idno1 = $rowverify["IDNO"];
				
				$qryscholarwaitingupdate = mysql_query("UPDATE division SET DIVCODE='$divcode',
																		MADEBY='$employeeid',
