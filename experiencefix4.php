...
$qryerror = mysql_query("SELECT * FROM crewchange WHERE DEPMNLDATE='1970-01-01'") or die(mysql_error());

$cnt = 1;
$cnt2 = 1;

while ($rowerror = mysql_fetch_array($qryerror))
{
	$ccid = $rowerror["CCID"];
...
//	if (!empty($rowerror["XDEPMNLDATE"]))
//		$xdepmnldate = "'" . date("Y-m-d",strtotime($rowerror["XDEPMNLDATE"])) . "'";
//	else 
//		$xdepmnldate = "NULL";
	
	if (!empty($rowerror["DEPMNLDATE"]))
		$depmnldate = "'" . date("Y-m-d",strtotime($rowerror["DEPMNLDATE"])) . "'";
	else 
		$depmnldate = "NULL";
...
//		$xdateemb = "NULL";
	...
	if (!empty($rowerror["DATEEMB"]))
		$dateemb = "'" . date("Y-m-d",strtotime($rowerror["DATEEMB"])) . "'";
	else 
		$dateemb = "NULL";
		
	$xdateemb2 = $rowerror["DATEEMB"];
...
//		$cnt2++;
	...
	$newdepmnldate = "'" . date("Y-m-d",strtotime("$xdateemb2 - 1 day")) . "'";
	
	$qryupdate = mysql_query("UPDATE crewchange SET DEPMNLDATE=$newdepmnldate WHERE CCID=$ccid") or die(mysql_error());
	
	echo "$cnt) CCID: $ccid -- Updated DEPMNLDATE from $depmnldate ----> $newdepmnldate <br />";
	$cnt++;
