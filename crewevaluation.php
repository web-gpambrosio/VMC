...
// $basedirdocs = "docimages/"; // changed 2010-02-10 by GPA
$basedirdocs = "docimg/";
...
// $serverdirtmp="docimages"; // changed 2010-02-10 by GPA
...
if (isset($_POST["nextrankcode"]))
	$nextrankcode = $_POST["nextrankcode"];
...
if (isset($_POST["totgrade"]))
	$totgrade = $_POST["totgrade"];
		...
		case "save":
			...
			// {
				...
				if ($allowsave > 0 || $finalgrade != "")
				{
					...
					if(empty($nextrankcode))
						$nextrankcode="NULL";
					else 
						$nextrankcode="'$nextrankcode'";
					...
					if(mysql_num_rows($qrychkdata)==0)
					{
						$getsave="INSERT INTO crewevalhdr (CCID,EVALNO,EVALDATE,VPC1,VPC2,VPC3,VPC4,VPC5,VPC6,VPC1REM,VPC2REM,
							...
							EPR1,EPRBY1,EPRDATE1,EPR2,EPRBY2,EPRDATE2,EPR3,EPRBY3,EPRDATE3,MADEBY,MADEDATE,FINALGRADE) VALUES ($ccid,$evalno,
							'$evaldate',$vpc1,$vpc2,$vpc3,$vpc4,$vpc5,$vpc6,$vpc1rem,$vpc2rem,$vpc3rem,$vpc4rem,
							$vpc5rem,$vpc6rem,$vee1,$vee1rem,$fleetkept,
							$vep0,$vep0rem,$nextrankcode,$vep1,$vep2,$vep3,$vep4,$negdiscussed,$var1,$var2,$var3,
							...
							'$datetimenow',$finalgrade)";
		//				echo $getsave;
						$qrysavedata=mysql_query("$getsave") or die(mysql_error());
					}
					else
					{
						$qryupdatecer = mysql_query("UPDATE crewevalhdr SET VPC1=$vpc1,VPC2=$vpc2,VPC3=$vpc3,VPC4=$vpc4,VPC5=$vpc5,VPC6=$vpc6,
													VPC1REM=$vpc1rem,VPC2REM=$vpc2rem,VPC3REM=$vpc3rem,VPC4REM=$vpc4rem,VPC5REM=$vpc5rem,VPC6REM=$vpc6rem,
													VEE1=$vee1,VEE1REM=$vee1rem,FLEETKEPT=$fleetkept,VEP0=$vep0,VEP0REM=$vep0rem,NEXTRANK=$nextrankcode,
													VEP1=$vep1,VEP2=$vep2,VEP3=$vep3,VEP4=$vep4,NEGDISCUSSED=$negdiscussed,
													VAR1=$var1,VAR2=$var2,VAR3=$var3,VAR4=$var4,VPJ1=$vpj1,VPJ2=$vpj2,VPJ3=$vpj3,VPJ4=$vpj4,
													VSM1=$vsm1,VSM2=$vsm2,VSM3=$vsm3,VPR1=$vpr1,VPR2=$vpr2,VPR3=$vpr3,VBE1=$vbe1,VBE2=$vbe2,VBE3=$vbe3,
													VSC1=$vsc1,VSC2=$vsc2,VSC3=$vsc3,CAPTNAME1=$captname1,DATE1=$date1,CAPTNAME2=$captname2,DATE2=$date2,
													PRINCIPALCOMMENT=$principalcomment,VMCCOMMENT=$vmccomment,EXTRATRAIN=$extratrain,REPEXTRATRAIN=$repextratrain,
													REPEXTRATRAINREM=$repextratrainrem,NEWTRAINCODE1=$newtraincode1,NEWTRAINOTH1=$newtrainoth1,
													NEWTRAINCODE2=$newtraincode2,NEWTRAINOTH2=$newtrainoth2,NEWTRAINCODE3=$newtraincode3,NEWTRAINOTH3=$newtrainoth3,
													EFE1=$efe1,EFEBY1=$efeby1,EFEDATE1=$efedate1,EFE2=$efe2,EFEBY2=$efeby2,EFEDATE2=$efedate2,EFE3=$efe3,
													EFEBY3=$efeby3,EFEDATE3=$efedate3,EPR1=$epr1,EPRBY1=$eprby1,EPRDATE1=$eprdate1,
													EPR2=$epr2,EPRBY2=$eprby2,EPRDATE2=$eprdate2,EPR3=$epr3,EPRBY3=$eprby3,EPRDATE3=$eprdate3,
													MADEBY='$user',MADEDATE='$datetimenow',FINALGRADE=$finalgrade
												WHERE CCID=$ccid AND EVALNO=$evalno
												") or die(mysql_error());
					}
				...
				{
					echo "<script>alert('Data Incomplete: Please check Grading(2nd Page). Then try again.');</script>";
				}
			// }
			// else 
			// {
				// echo "<script>alert('Data exists! Saving failed.');</script>";
			// }
		break;
		case  "uploadpdf":
...
//				$xclgname = "XXX";
	...
	//				$xclnegdiscussed=$data->sheets[$Sheet]['cells'][57][5]; //NEXT RANK
					...
					$vep1=$data->sheets[$Sheet]['cells'][55][30]; //VEP1
					$vep2=$data->sheets[$Sheet]['cells'][56][30]; //VEP2
					$vep3=$data->sheets[$Sheet]['cells'][57][30]; //VEP3
					$vep4=$data->sheets[$Sheet]['cells'][58][30]; //VEP4
					...
					//find rankcode
					$qryfind=mysql_query("SELECT RANKCODE FROM rank WHERE ALIAS1='$nextrank' OR ALIAS2='$nextrank'") or die(mysql_error());
					if(mysql_num_rows($qryfind)==1)
					{
						$rowfind=mysql_fetch_array($qryfind);
						$nextrankcode=$rowfind["RANKCODE"];
					}
					else 
						$nextrankcode="";
...
$qrygethigherrank=mysql_query("SELECT RANKCODE,ALIAS2
	FROM rank
	WHERE STATUS=1 AND ALIAS2 <> ''") or die(mysql_error());
//$qrygethigherrank=mysql_query("SELECT RANKCODE,ALIAS2 
//	FROM rank
...
if ($fordeletion==1)
{
	$qryevaldelete = mysql_query("DELETE FROM crewevalhdr WHERE CCID=$ccid AND EVALNO=$evalno") or die(mysql_error());

	$dirfilenamepdf = $basedirdocs . $applicantno . "/CER/" . $ccid . "_" . $evalno . ".pdf";
	$dirfilenamexls = $basedirdocs . $applicantno . "/CER/" . $ccid . "_" . $evalno . ".xls";

	if (checkpath($dirfilenamepdf,1))
	{
		//delete file
		unlink($dirfilenamepdf);
	}

	if (checkpath($dirfilenamexls,1))
	{
		//delete file
		unlink($dirfilenamexls);
	}
}	
...
if($uploaded!=1)
{
	...
	if(mysql_num_rows($qrygetevaldetails)!=0)
	{
		...
		$nextrankcode = $rowdtls["NEXTRANK"];
		//get alias2
		$qryalias=mysql_query("SELECT ALIAS2 FROM rank WHERE RANKCODE='$nextrankcode'") or die(mysql_error());
		$rowalias=mysql_fetch_array($qryalias);
		$nextrank=$rowalias["ALIAS2"];
		$vep1 = $rowdtls["VEP1"];
		$vep2 = $rowdtls["VEP2"];
		$vep3 = $rowdtls["VEP3"];
		$vep4 = $rowdtls["VEP4"];
...
$btndel="<input type=\"button\" $disableall 			onclick=\"if (confirm('This will delete Eval No. $evalno, and all PDF/XLS files associated with it. Confirm?'))actiontxt.value='delete';crewevaluation.submit();\" value=\"Delete\">";
...
<form name=\"crewevaluation\" id=\"crewevaluation\" method=\"POST\" enctype=\"multipart/form-data\">\n
	...
	echo "
		<table cellspacing=\"0\" cellpadding=\"0\" width=\"$tablewidth\" 
			style=\"font-family:Arial;font-size:0.8em;\">
			<tr>
				<td width=\"30px\">$btnnext</td>
				<td width=\"30px\">$btnsave</td>
				<td width=\"16%\">$btnviewpdf</td>
				<td style=\"text-align:right;\">
					...
					<span style=\"font-size:0.9em;font-style:italic;color:Blue;\">
						Note: Please choose EVAL NO(1,2,3,4) first, then fill-up Final Grade, then click SAVE.<br />
						Don't forget to UPLOAD the scanned CER, in PDF format.
					</span>
					<br />
					<b>Final Grade</b>
					<input type=\"text\" name=\"finalgrade\" size=\"7\" style=\"font-weight:bold;color:Red;text-align:center;\" value=\"$finalgrade\" />
		...
		<table cellspacing=\"0\" cellpadding=\"0\" width=\"$tablewidth\" 
			...
			<tr height=\"16px\">\n
				<td colspan=\"21\">&nbsp;</td>\n
				<td style=\"border-bottom:1px dotted black;border-right:1px dotted black;border-left:1px dotted black;\"><input type=\"radio\" name=\"evalnoradio\" $evalno1 onclick=\"finalgrade.value='';crewevaluation.submit();\" value=\"evalno1\"></td>\n
				<td colspan=\"17\">&nbsp;&nbsp;1.&nbsp;&nbsp;TO BE EVALUATED 4 MONTHS AFTER EMBARKATION</td>\n
			</tr>
			<tr height=\"16px\">\n
				<td>&nbsp;</td>\n
				<td colspan=\"5\">SHIP NAME</td>\n
				<td colspan=\"14\" style=\"font-weight:bold;\">$vesselname</td>\n
				<td>&nbsp;</td>\n
				<td style=\"border-bottom:1px dotted black;border-right:1px dotted black;border-left:1px dotted black;\"><input type=\"radio\" name=\"evalnoradio\" $evalno2 onclick=\"finalgrade.value='';crewevaluation.submit();\" value=\"evalno2\"></td>\n
				<td colspan=\"17\">&nbsp;&nbsp;2.&nbsp;&nbsp;TO BE EVALUATED 1 MONTH BEFORE DISEMBARKING</td>\n
			</tr>
			<tr height=\"16px\">\n
				<td colspan=\"21\">&nbsp;</td>\n
				<td style=\"border-bottom:1px dotted black;border-right:1px dotted black;border-left:1px dotted black;\"><input type=\"radio\" name=\"evalnoradio\" $evalno3 onclick=\"finalgrade.value='';crewevaluation.submit();\" value=\"evalno3\"></td>\n
				<td colspan=\"8\" style=\"text-align:left;\">&nbsp;&nbsp;3.&nbsp;&nbsp;SIGN OFF MASTER</td>\n
				<td style=\"border-top:1px dotted black;border-bottom:1px dotted black;border-right:1px dotted black;border-left:1px dotted black;\"><input type=\"radio\" name=\"evalnoradio\" $evalno4 onclick=\"finalgrade.value='';crewevaluation.submit();\" value=\"evalno4\"></td>\n
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vpc1\" size=\"1\" value=\"$vpc1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;font-weight:bold;border:0;text-align:center;\" 
				...
				<td colspan=\"19\" style=\"text-align:center;border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" 
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;\" 
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vpc2\" size=\"1\"  value=\"$vpc2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vpc3\" size=\"1\"  value=\"$vpc3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vpc4\" size=\"1\"  value=\"$vpc4\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vpc5\" size=\"1\" value=\"$vpc5\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px solid black;\">
					<input type=\"text\" name=\"vpc6\" size=\"1\" value=\"$vpc6\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"2\" style=\"border-left:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"newtraincode1\" size=\"1\" value=\"$newtraincode1\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
				...
				<td colspan=\"15\" style=\"border-right:1px dotted black;\">
					<select $disableall name=\"newtrain1\" onclick=\"newtraincode1.value=this.value;\"
			...
			<tr height=\"16px\">\n
				<td colspan=\"2\">&nbsp;</td>\n
				<td colspan=\"2\" style=\"border-left:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"newtraincode2\" size=\"1\" value=\"$newtraincode2\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
						onblur=\"if(this.value!=''){selecttrainingcode(this.value,2)}else{newtrain2.value=''}\"
						style=\"font-size:8pt;border:0;$bgcolor;font-weight:bold;text-align:center;\">
				</td>\n
				<td colspan=\"15\" style=\"border-right:1px dotted black;\">
					<select $disableall name=\"newtrain2\" onclick=\"newtraincode2.value=this.value;\"
						style=\"font-size:8pt;border:0;width:280px;border-style:outset;\">
						<option value=\"\"></option>
						$getselecttraining2
					</select>
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"2\" style=\"border-bottom:1px solid black;border-left:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"newtraincode3\" size=\"1\" value=\"$newtraincode3\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
				...
				<td colspan=\"15\" style=\"border-bottom:1px solid black;border-right:1px dotted black;\">
					<select $disableall name=\"newtrain3\" onclick=\"newtraincode3.value=this.value;\"
						style=\"font-size:8pt;border:0;width:280px;border-style:outset;\">
						<option value=\"\"></option>
						$getselecttraining3
					</select>
			...
			<tr height=\"16px\">\n
				...
				<td>
					<input type=\"text\" name=\"vee1\" size=\"1\" value=\"$vee1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-left:1px dotted black;border-top:1px dotted black;border-right:1px dotted black;\">
					<input type=\"text\" name=\"fleetkept\" size=\"1\" value=\"$fleetkept\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-left:1px solid black;border-top:1px solid black;border-bottom:1px solid black;\">
					<input type=\"text\" name=\"vep0\" size=\"1\" value=\"$vep0\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vep1\" size=\"1\" value=\"$vep1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vep2\" size=\"1\" value=\"$vep2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"6\" rowspan=\"2\" style=\"border-bottom:1px solid black;text-align:center;\">
					<select $disableall name=\"nextrank\" style=\"font-size:14pt;\" onchange=\"document.getElementById('nxtrank').innerHTML=this.value;\">
						...
						{
							$nextrankcode1=$rowgethigherrank["RANKCODE"];
							$nextrankalias=$rowgethigherrank["ALIAS2"];
							if($nextrankcode1==$nextrankcode)
								$selected="selected";
							else
								$selected="";
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dotted black;\">
					<input type=\"text\" name=\"vep3\" size=\"1\" value=\"$vep3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px solid black;\">
					<input type=\"text\" name=\"vep4\" size=\"1\" value=\"$vep4\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"12\" style=\"border-top:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"captname1\" size=\"40\" value=\"$captname1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;\" 
						onkeydown=\"if(event.keyCode==13){}\">
				</td>\n
				...
				<td colspan=\"11\" style=\"border-right:1px solid black;border-top:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"captname2\" size=\"40\" value=\"$captname2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;\" 
						onkeydown=\"if(event.keyCode==13){}\">
				</td>\n
			</tr>
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"7\" style=\"text-align:center;border-bottom:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"date1\" size=\"20\" value=\"$date1\"
				...
				<td colspan=\"7\" style=\"text-align:center;border-bottom:1px solid black;border-right:1px solid black;\">&nbsp;
					<input type=\"text\" name=\"date2\" size=\"20\" value=\"$date2\"
						onkeydown=\"if(event.keyCode==13){if(this.value!=''){event.keyCode=9;}}\"
						onblur=\"chkdate(this);\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
		...
		<table cellspacing=\"0\" cellpadding=\"0\" width=\"$tablewidth\" 
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"var1\" size=\"1\" value=\"$var1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"var2\" size=\"1\" value=\"$var2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"var3\" size=\"1\" value=\"$var3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-right:1px solid black;\">
					<input type=\"text\" name=\"var4\" size=\"1\" value=\"$var4\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpj1\" size=\"1\" value=\"$vpj1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpj2\" size=\"1\" value=\"$vpj2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpj3\" size=\"1\" value=\"$vpj3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpj4\" size=\"1\" value=\"$vpj4\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vsm1\" size=\"1\" value=\"$vsm1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vsm2\" size=\"1\" value=\"$vsm2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vsm3\" size=\"1\" value=\"$vsm3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpr1\" size=\"1\" value=\"$vpr1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpr2\" size=\"1\" value=\"$vpr2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vpr3\" size=\"1\" value=\"$vpr3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vbe1\" size=\"1\" value=\"$vbe1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px dotted black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vbe2\" size=\"1\" value=\"$vbe2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-bottom:1px solid black;border-right:1px solid black;\">
					<input type=\"text\" name=\"vbe3\" size=\"1\" value=\"$vbe3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td colspan=\"4\" rowspan=\"2\" style=\"font-size:10pt;font-weight:bold;text-align:center;border-top:2px solid black;border-bottom:2px solid black;\"><input type=\"text\" size=\"5\" style=\"border:0;font-family:Arial;font-weight:bold;font-size:10pt;\" id=\"totalscore\" name=\"totgrade\"></td>\n
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dashed black;\">
					<input type=\"text\" name=\"vsc1\" size=\"1\" value=\"$vsc1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px dashed black;\">
					<input type=\"text\" name=\"vsc2\" size=\"1\" value=\"$vsc2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				...
				<td style=\"border-right:1px solid black;border-bottom:1px solid black;\">
					<input type=\"text\" name=\"vsc3\" size=\"1\" value=\"$vsc3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\" 
						onkeypress=\"return numbersonly(this);\"
						onkeydown=\"if(event.keyCode==13){event.keyCode=9;}\"
			...
			<tr height=\"16px\">\n
				<td colspan=\"2\">&nbsp;</td>\n
				<td colspan=\"20\" style=\"font-size:1em;\">Total Evaluation for Further Employment</td>\n
				<td colspan=\"15\" style=\"font-size:1em;\"><u>Additional Comment, if any </u></td>\n
			</tr>
			<tr height=\"16px\">\n
				<td colspan=\"2\">&nbsp;</td>\n
				...
				<td colspan=\"3\" style=\"text-align:center;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"efeby1\" size=\"6\" value=\"$efeby1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td colspan=\"3\" style=\"text-align:right;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"efeby2\" size=\"6\" value=\"$efeby2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td colspan=\"3\" style=\"text-align:right;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"efeby3\" size=\"6\" value=\"$efeby3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td>&nbsp;</td>\n
			...
			<tr height=\"16px\">\n
				<td colspan=\"2\">&nbsp;</td>\n
				<td colspan=\"10\" style=\"font-size:1em;\">Evaluation for Promotion to rank (</td>\n
				<td colspan=\"3\" style=\"font-size:1em;text-align:center;\"><span id=\"nxtrank\" style=\"font-weight:bold;text-align:center;\">&nbsp;</span></td>\n
				<td colspan=\"7\" style=\"font-size:1em;\">)</td>\n
			</tr>
			<tr height=\"16px\">\n
				<td colspan=\"2\">&nbsp;</td>\n
				<td colspan=\"10\" style=\"text-align:right;border:1px solid black;\">Judged by:&#x2192;</td>\n
				<td colspan=\"3\" style=\"text-align:right;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"eprby1\" size=\"6\" value=\"$eprby1\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td colspan=\"3\" style=\"text-align:center;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"eprby2\" size=\"6\" value=\"$eprby2\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td colspan=\"3\" style=\"text-align:center;border:1px solid black;border-left:0;\">
					<input type=\"text\" name=\"eprby3\" size=\"6\" value=\"$eprby3\"
						$disableall style=\"font-size:1em;$bgcolor;font-weight:bold;border:0;text-align:center;\">
				</td>\n
				<td>&nbsp;</td>\n
	...
	<input type=\"hidden\" name=\"nextrankcode\" value=\"$nextrankcode\">
	<script>
		codefunc('vpc1',4);
		codefunc('vpc2',4);
		codefunc('vpc3',4);
