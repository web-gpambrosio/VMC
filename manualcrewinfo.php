...
if (isset($_POST["familykin"]))
	$familykin = $_POST["familykin"];
...
if (isset($_POST["injurydate"]))
	$injurydate = $_POST["injurydate"];

if (isset($_POST["injuryreason"]))
	$injuryreason = $_POST["injuryreason"];

if (isset($_POST["injuryclinic"]))
	$injuryclinic = $_POST["injuryclinic"];

if (isset($_POST["injurychkupdate"]))
	$injurychkupdate = $_POST["injurychkupdate"];

if (isset($_POST["injurydiagnosis"]))
	$injurydiagnosis = $_POST["injurydiagnosis"];

if (isset($_POST["injuryrecommendcode"]))
	$injuryrecommendcode = $_POST["injuryrecommendcode"];
...
// if (isset($_POST["watchpoearem"]))
	// $watchpoearem = $_POST["watchpoearem"];
	
if (isset($_POST["watchvmcrem"]))
	$watchvmcrem = $_POST["watchvmcrem"];
...
//											";
				...
				case "8": //MEDICAL
						...
						if (!empty($medcheckupdate))
							$medcheckupdateraw = "'" . date('Y-m-d',strtotime($medcheckupdate)) . "'";
						else
							$medcheckupdateraw = "NULL";
							
						if (empty($medrecommendcode))
							$medrecommendcode = "NULL";
						else
							$medrecommendcode = "'" . $medrecommendcode . "'";
				...
				case "11": //INJURY
					...
					if (!empty($injurydate))
						$injurydateraw = "'" . date('Y-m-d',strtotime($injurydate)) . "'";
					else
						$injurydateraw = "NULL";
		
					if (!empty($injurychkupdate))
						$injurychkupdateraw = "'" . date('Y-m-d',strtotime($injurychkupdate)) . "'";
					else
						$injurychkupdateraw = "NULL";
				
					$qrychkinjury = mysql_query("SELECT CCID FROM crewinjury WHERE CCID=$editid") or die(mysql_error());
					
					if (mysql_num_rows($qrychkinjury) > 0)
					{ 
						$qryinjuryupdate = mysql_query("UPDATE crewinjury SET 
															DATEINJURED = $injurydateraw,
															REASON = '$injuryreason',
															MADEBY = '$employeeid',
															MADEDATE = '$currentdate'
												WHERE CCID = $editid
						...
						if (!empty($injuryrecommendcode))
						{
							$qrychkinjurychkup = mysql_query("SELECT IDNO FROM crewinjurychkup WHERE CCID=$editid") or die(mysql_error());
					
							if (mysql_num_rows($qrychkinjurychkup) > 0)
							{
								$rowchkinjurychkup = mysql_fetch_array($qrychkinjurychkup);
								$chkupidno = $rowchkinjurychkup["IDNO"];
					
								$qryinjurychkupdate = mysql_query("UPDATE crewinjurychkup SET
																	CLINICNAME = '$injuryclinic',
																	DATECHECKUP = $injurychkupdateraw,
																	DIAGNOSIS = '$injurydiagnosis',
																	RECOMMENDCODE = '$injuryrecommendcode', 
																	MADEBY = '$employeeid',
																	MADEDATE = '$currentdate'
															WHERE IDNO=$chkupidno
													") or die(mysql_error());
							}
							else
							{
								$qryinjurychkinsert = mysql_query("INSERT INTO crewinjurychkup VALUES(NULL,$editid,'$injuryclinic',$injurychkupdateraw,
														'$injurydiagnosis','$injuryrecommendcode','$employeeid','$currentdate')
													") or die(mysql_error());
							}
						}
					}
					else  //INSERT INTO crewinjury (NEW ENTRY)
					{
						$qryinjurysave = mysql_query("INSERT INTO crewinjury VALUES($editid,$injurydateraw,'$injuryreason','$employeeid','$currentdate')") or die(mysql_error());
			...
			switch ($currentpage)
			{
				...
				case "1": //FAMILY
						...
						$qryfamilydata = mysql_query("SELECT RELCODE,FNAME,GNAME,MNAME,ADDRESS,PROVINCECODE,TOWNCODE,BARANGAYCODE,
												...
												OCCUPATION,DEPENDENT,OTHERRELATION,NEXTOFKIN,
												GENDER,ABROAD,EMAIL 
												FROM crewfamily 
						...
						$familykin = $rowfamilydata["NEXTOFKIN"];
...
//						$empovertime = $rowempdata["OVERTIME"];
				...
				case "11":  //INJURY
					...
					$qryinjurydata = mysql_query("SELECT ci.CCID,ci.DATEINJURED,ci.REASON,cic.CLINICNAME,cic.DATECHECKUP,cic.DIAGNOSIS,cic.RECOMMENDCODE
										FROM crewinjury ci
										LEFT JOIN crewinjurychkup cic ON cic.CCID=ci.CCID
										WHERE ci.CCID=$editid
								") or die(mysql_error());
								
					$rowinjurydata = mysql_fetch_array($qryinjurydata);
				
					if (!empty($rowinjurydata["DATEINJURED"]))
						$injurydate = date('m/d/Y',strtotime($rowinjurydata["DATEINJURED"]));
					else
						$injurydate = "";
						
					$injuryreason = $rowinjurydata["REASON"];
					$injuryclinic = $rowinjurydata["CLINICNAME"];
					
					if (!empty($rowinjurydata["DATECHECKUP"]))
						$injurychkupdate = date('m/d/Y',strtotime($rowinjurydata["DATECHECKUP"]));
					else	
						$injurychkupdate = "";
						
					$injurydiagnosis = $rowinjurydata["DIAGNOSIS"];
					$injuryrecommendcode = $rowinjurydata["RECOMMENDCODE"];
	...
	case "addlist"	:  //IF "ADD TO LIST" BUTTON IS CLICKED -- SAVE DETAILS
			...
			switch ($currentpage)
			{
				...
				case "1": //FAMILY
						...
						if (mysql_num_rows($qryfamilycheck) < 1)
						{
							...
							$qryfamilysave = mysql_query("INSERT INTO crewfamily(CREWRELID,APPLICANTNO,RELCODE,FNAME,GNAME,MNAME,
											...
											'$familygender',$familyabroad,'$familyemail',$familykin)
											") or die(mysql_error());
				...
				case "5": //CERTIFICATE
						...
						if (!empty($certno))
						{
							...
							if (mysql_num_rows($qrycertcheck) < 1)
							{
								...
								// echo "INSERT INTO crewcertstatus(APPLICANTNO,RANKCODE,DOCCODE,DOCNO,DATEISSUED,MADEBY,MADEDATE)
				...
				// case "11": //INJURY
							...
							// $qryinjurysave = mysql_query("INSERT INTO crewinjury VALUES(CCID,'$injurydate','$injuryreason','$employeeid','$currentdate')") or die(mysql_error());
		...
		break;
	case "liftwithdrawal":
		mysql_query("UPDATE crewwithdrawal SET LIFTWITHDRAWAL=1 WHERE IDNO=$editid") or die(mysql_error());
	break;
	case "addliftwithdrawal":
		$safekeepremarks=mysql_real_escape_string($_POST["safekeepremarks"]);
		mysql_query("INSERT INTO crewwithdrawal (APPLICANTNO,FORMDATE,SAFEKEEPREMARKS,SAFEKEEPBY,SAFEKEEPDATE,NFR) VALUES('$applicantno','$currentdate','$safekeepremarks','$employeeid','$currentdate',1)") or die(mysql_error());
	break;
	case "deceased":
		$deceasedreason=mysql_real_escape_string($_POST["deceasedreason"]);
		$datedeceased=date("Y-m-d",strtotime($_POST["datedeceased"]));
		$qrychkexist=mysql_query("SELECT APPLICANTNO FROM crewdeceased WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		if(mysql_num_rows($qrychkexist)==0)
			mysql_query("INSERT INTO crewdeceased (APPLICANTNO,DATEDECEASED,REASON,MADEBY,MADEDATE) VALUES('$applicantno','$datedeceased','$deceasedreason','$employeeid','$currentdate')") or die(mysql_error());
		else 
			mysql_query("UPDATE crewdeceased SET DATEDECEASED='$datedeceased',REASON='$deceasedreason',MADEBY='$employeeid',MADEDATE='$currentdate'
				WHERE APPLICANTNO=$applicantno") or die(mysql_error());
	break;
	case "changestatus":
		$qrychkstatus = mysql_query("SELECT STATUS FROM crew WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		
		$rowchkstatus = mysql_fetch_array($qrychkstatus);
		$xstatus = $rowchkstatus["STATUS"];
		...
		$qryupdatestatus = mysql_query("UPDATE crew SET STATUS=$xstatus,MADEBY='$employeeid',MADEDATE='$currentdate' WHERE APPLICANTNO=$applicantno") or die(mysql_error());
	break;
	case "changeutility":
		$qrychkutility = mysql_query("SELECT UTILITY FROM crew WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		
		$rowchkutility = mysql_fetch_array($qrychkutility);
		$utilstatus = $rowchkutility["UTILITY"];
		
		if ($utilstatus == 0)
		{
			$qryupdateutility = mysql_query("UPDATE crew SET UTILITY=1 WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		}
		else
		{
			$qryupdateutility = mysql_query("UPDATE crew SET UTILITY=0 WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		}
		
	break;
	case "updatewatchlist":
		
		$qrygetcrewinfo = mysql_query("SELECT FNAME,GNAME,MNAME,SSS,TIN FROM crew WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		$rowgetcrewinfo = mysql_fetch_array($qrygetcrewinfo);
		
		$wfname = $rowgetcrewinfo["FNAME"];
		$wgname = $rowgetcrewinfo["GNAME"];
		$wmname = $rowgetcrewinfo["MNAME"];
		$wsss = $rowgetcrewinfo["SSS"];
		$wtin = $rowgetcrewinfo["TIN"];
		
		// if(!empty($poeaidno))
			// $qryupdatewatchpoea = mysql_query("UPDATE watchlist_poea SET REMARKS='$watchpoearem' WHERE IDNO=$poeaidno") or die(mysql_error());
		// else
		// {
			// $qryinsertwatchpoea = mysql_query("INSERT INTO watchlist_poea(FNAME,GNAME,MNAME,SSS,TIN,DATEFILED,REMARKS,MADEBY,MADEDATE) 
						// VALUES('$wfname','$wgname','$wmname','$wsss','$wtin','$currentdate',$watchvmcrem','$currentdate','$employeeid')
						// ") or die(mysql_error());
		// }
		
		if(!empty($vmcidno))
			$qryupdatewatchvmc = mysql_query("UPDATE watchlist_veritas SET REMARKS='$watchvmcrem' WHERE IDNO=$vmcidno") or die(mysql_error());
		else
		{
			// $qrygetcrewinfo = mysql_query("SELECT FNAME,GNAME,MNAME,SSS,TIN FROM crew WHERE APPLICANTNO=$applicantno") or die(mysql_error());
			// $rowgetcrewinfo = mysql_fetch_array($qrygetcrewinfo);
			...
			$qryinsertwatchvmc = mysql_query("INSERT INTO watchlist_veritas(FNAME,GNAME,MNAME,APPLICANTNO,SSS,TIN,DATEFILED,REMARKS,MADEBY,MADEDATE) 
						VALUES('$wfname','$wgname','$wmname','$applicantno','$wsss','$wtin','$currentdate','$watchvmcrem','$employeeid','$currentdate')
						") or die(mysql_error());
		}
	break;
	case "secondchance":
		
		$qryverifypoea = mysql_query("SELECT IDNO,SECONDCHANCE FROM watchlist_poea WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		
		if (mysql_num_rows($qryverifypoea) > 0)
		{
			$rowverifypoea = mysql_fetch_array($qryverifypoea);
			$poeaidno = $rowverifypoea["IDNO"];
			if ($rowverifypoea["SECONDCHANCE"] == 0)
				$psecondchance = 1;
			else
				$psecondchance = 0;
		}
		
		$qryverifyvmc = mysql_query("SELECT IDNO,SECONDCHANCE FROM watchlist_veritas WHERE APPLICANTNO=$applicantno") or die(mysql_error());
		
		if (mysql_num_rows($qryverifyvmc) > 0)
		{
			$rowverifyvmc = mysql_fetch_array($qryverifyvmc);
			$vmcidno = $rowverifyvmc["IDNO"];
			if ($rowverifyvmc["SECONDCHANCE"] == 0)
				$vsecondchance = 1;
			else
				$vsecondchance = 0;
		}
		...
		if(!empty($poeaidno))
			$qrysecondchance1 = mysql_query("UPDATE watchlist_poea SET SECONDCHANCE=$psecondchance WHERE IDNO=$poeaidno") or die(mysql_error());
			
		if(!empty($vmcidno))
			$qrysecondchance2 = mysql_query("UPDATE watchlist_veritas SET SECONDCHANCE=$vsecondchance WHERE IDNO=$vmcidno") or die(mysql_error());
	...
	if ($applicantno != "")
	{
		$qrygetcrew = mysql_query("SELECT FNAME,GNAME,MNAME,ADDRESS,PROVINCECODE,TOWNCODE,BARANGAYCODE,ZIPCODE,TELNO,
									...
									CEL1,CEL2,CEL3,CREWCODE,c.STATUS,
									...
									IF (cf.APPLICANTNO IS NOT NULL,f.FASTTRACK,'') AS FASTTRACK,c.UTILITY
									FROM crew c
...
//					$crewnationality = $rowgetcrew["NATIONALITY"];
		...
		if ($rowgetcrew["STATUS"] == 1)
			$crewstatus = "ACTIVE";
		else
			$crewstatus = "INACTIVE";
			
		$crewutility = $rowgetcrew["UTILITY"];
...
//+++++++++++++++  FAMILY  ++++++++++++++++++//
	...
	$familyselect6a = "<option selected value=\"\">--Select--</option>";
	
	$familykin1 = "family5" . $familykin;
	$$familykin1 = "SELECTED";	
	
	$familyselect6a .= "<option $family51 value=\"1\">YES</option>";
	$familyselect6a .= "<option $family50 value=\"0\">NO</option>";
...
$qryrecommend2 = mysql_query("SELECT RECOMMENDCODE,RECOMMENDATION FROM clinicrecommend") or die(mysql_error());
$injuryselect = "<option selected value=\"\">--Select One--</option>";
	while($rowrecommend2=mysql_fetch_array($qryrecommend2))
	{
		$recommendcode2 = $rowrecommend2["RECOMMENDCODE"];
		$recommendation2= $rowrecommend2["RECOMMENDATION"];
		
		$selected = "";
		
		if ($injuryrecommendcode == $recommendcode2)
			$selected = "SELECTED";
			
		$injuryselect .= "<option $selected value=\"$recommendcode2\">$recommendation2</option>";
	}
...
if ($applicantno != "")
{
	...
	$qryfamilylist = mysql_query("SELECT CREWRELID,CONCAT(FNAME,', ',GNAME,' ',MNAME) AS NAME,CONCAT(ADDRESS,', ',TOWN,' ',PROVINCE) AS ADDRESS2,
								cf.ADDRESS,cf.MUNICIPALITY,cf.CITY,
								fr.RELATION,TELNO,DEPENDENT,ABROAD,NEXTOFKIN
								FROM crewfamily cf
	...
	$qrymedicallist = mysql_query("
				SELECT cm.IDNO,cm.CLINICID,c.CLINIC,cm.DATECHECKUP,cm.DIAGNOSIS,cr.RECOMMENDATION,cm.REMARKS,cm.HBATEST,cm.DOCCODE
				FROM crewmedical cm
				...
				ORDER BY cm.DATECHECKUP DESC") or die(mysql_error());
	
	$qrypulloutlist = mysql_query("
				SELECT IDNO,FORMDATE,SAFEKEEPREMARKS,SAFEKEEPBY,SAFEKEEPDATE,NFR,LIFTWITHDRAWAL
				FROM crewwithdrawal
				WHERE APPLICANTNO=$applicantno
				ORDER BY FORMDATE DESC
				") or die(mysql_error());
	
	$qrydeceasedlist=mysql_query("SELECT * FROM crewdeceased WHERE APPLICANTNO=$applicantno") or die(mysql_error());
	
	$qryexpinjurylist=mysql_query("
				SELECT cc.CCID,cc.DATEEMB,IF(DATECHANGEDISEMB IS NULL,DATEDISEMB,DATECHANGEDISEMB) AS DATEDISEMB,
				v.VESSEL,v.ALIAS1,LEFT(v.VESSEL,10) AS VESSELSHORT,
				ci.DATEINJURED,ci.REASON,cic.CLINICNAME,cic.DATECHECKUP,cic.DIAGNOSIS,cr.RECOMMENDATION
				FROM crewchange cc
				LEFT JOIN vessel v ON v.VESSELCODE=cc.VESSELCODE
				LEFT JOIN crewinjury ci ON ci.CCID=cc.CCID
				LEFT JOIN crewinjurychkup cic ON cic.CCID=ci.CCID
				LEFT JOIN clinicrecommend cr ON cr.RECOMMENDCODE=cic.RECOMMENDCODE
				WHERE cc.APPLICANTNO=$applicantno AND cc.DATEEMB < CURRENT_DATE AND cc.DEPMNLDATE IS NOT NULL
				ORDER BY cc.CCID DESC
				") or die(mysql_error());
...
<link rel=\"stylesheet\" href=\"veripro.css\">
	...
	<script>
		...
		var displaypage=new Array('personal','family','education','documents','license',
									'certificate','experience','employment','medical','pullout','deceased','injury');
...
//								rem=rem + ',Date Expired';
		...
		function changetab()
		{
			document.crewinfo.actiontxt.value='cancel';
			document.crewinfo.submit();
		}
	</script>
	...
	<div style=\"width:100%;\">
		...
		<table style=\"width:80%;margin-top:5px;\" border=1>
			<tr>
				...
				if($currentpage!=0)
					$disabledpage="disabled=\"disabled\"";
				...
				<td align=\"center\"><input type=\"text\" name=\"searchkey\" size=\"20\" value=\"$searchkey\" 
					...
					<input type=\"button\" name=\"btnfind\" value=\"Find\" onclick=\"currentpage.value='0';actiontxt.value='find';submit();\" />
					
					<input type=\"button\" name=\"btnreset\" value=\"Reset All\" onclick=\"actiontxt.value='reset';currentpage.value=0;submit();\" />
					<input type=\"button\" value=\"Save\" $showsave $disableapproved $disabledpage onclick=\"actiontxt.value='save';checkadd(currentpage.value);\" />
					";
		...
		<div style=\"width:36%;height:40px;float:left;background-color:Black;text-align:left;padding:0 0 0 5;\">
		...
		<div style=\"width:63%;height:35px;float:right;background-color:Gray;color:White;text-align:right;\">
		";
			$stylebtn = "style=\"font-size:0.55em;font-weight:Bold;border:0;background-color:Green;color:White;cursor:pointer;\"";
			
		echo "
			<input type=\"button\" value=\"PERSONAL\" $stylebtn onclick=\"changepage(1,currentpage.value,0);changetab();\" />
			<input type=\"button\" value=\"FAMILY\" $stylebtn onclick=\"changepage(1,currentpage.value,1);changetab();\" />
			<input type=\"button\" value=\"EDUCATION\" $stylebtn onclick=\"changepage(1,currentpage.value,2);changetab();\" />
			<input type=\"button\" value=\"DOCUMENTS\" $stylebtn onclick=\"changepage(1,currentpage.value,3);changetab();\" />
			<input type=\"button\" value=\"LICENSES\" $stylebtn onclick=\"changepage(1,currentpage.value,4);changetab();\" />
			<input type=\"button\" value=\"CERTIFICATES\" $stylebtn onclick=\"changepage(1,currentpage.value,5);changetab();\" />
			<input type=\"button\" value=\"EXP(OUTSIDE)\" $stylebtn onclick=\"changepage(1,currentpage.value,6);changetab();\" />
			<input type=\"button\" value=\"EMPLOYMENT\" $stylebtn onclick=\"changepage(1,currentpage.value,7);changetab();\" />
			<input type=\"button\" value=\"MEDICAL\" $stylebtn onclick=\"changepage(1,currentpage.value,8);changetab();\" />
			<input type=\"button\" value=\"PULLOUT\" $stylebtn onclick=\"changepage(1,currentpage.value,9);changetab();\" />
			<input type=\"button\" value=\"DECEASED\" $stylebtn onclick=\"changepage(1,currentpage.value,10);changetab();\" />
			<input type=\"button\" value=\"INJURY\" $stylebtn onclick=\"changepage(1,currentpage.value,11);changetab();\" />
		</div>
			...
			<table class=\"listrow\" width=\"100%\">
				...
				<tr>
					...
					if (!empty($applicantno))
					{
						...
						$listed = 0;
						$watchlistremarks = "";
						$watchlistremarks_vmc = "";
						$watchlistremarks_poea = "";
						$listed = checkwatchlist($applicantno,$watchlistremarks,$vmc_id,$poea_id,$watchlistremarks_poea,$watchlistremarks_vmc); 
							
						switch ($listed)
						{
							case 0	: $showlisted = "NO WATCHLIST"; break;
						...
						if (!empty($showlisted))
						{
							echo "<span style=\"color:Yellow;background-color:Black;font-size:1.5em;font-weight:Bold;\">&nbsp;&nbsp;&nbsp;$showlisted&nbsp;&nbsp;&nbsp;</span><br />
								VMC :&nbsp;<input type=\"text\" name=\"watchvmcrem\" size=\"20\" value=\"$watchlistremarks_vmc\" />
								&nbsp;&nbsp;<a href=\"#\" onclick=\"if (watchvmcrem.value != ''){
											if (confirm('This will toggle value of Veritas Watchlist Second Chance status. Confirm?')){
												vmcidno.value='$vmc_id';actiontxt.value='secondchance';crewinfo.submit();
												}
											}\" 
												title=\"Click to set Second Chance...\">- 2nd Chance -
										</a><br />

								<input type=\"button\" value=\"Update Remarks\" onclick=\"actiontxt.value='updatewatchlist';vmcidno.value='$vmc_id';crewinfo.submit();\" />
							";
						}
						else
						{
							echo "<span style=\"color:Green;font-size:1em;font-weight:Bold;\"><u>NO WATCHLIST</u></span>";
						}
					...
					<th align=\"center\">
						<br />
						<span style=\"color:Magenta;font-size:2em;font-weight:Bold;\">$crewstatus</span> <br />
						<a href=\"#\" onclick=\"actiontxt.value='changestatus';crewinfo.submit();\" style=\"font-size:1.2em;color:Green;\" title=\"Click to toggle status...\">-- Change Status --</a>
					";
...
echo "			<center><b>[NO PICTURE]</b></center>";
			...
			$styleutility = "style=\"border:0;background-color:Yellow;color:Blue;font-weight:Bold;font-size:0.9em;cursor:pointer;\"";
			...
			if ($crewutility == 1)
			{
				...
				<br />
				<center>
				<input type=\"button\" value=\"UTILITY\" onclick=\"actiontxt.value='changeutility';crewinfo.submit();\" title=\"Change to NOT UTILITY...\" $styleutility/>
				</center>
				";
			}
			else
			{
				...
				<br />
				<center>
				<input type=\"button\" value=\"NOT UTILITY\" onclick=\"actiontxt.value='changeutility';crewinfo.submit();\" title=\"Change to UTILITY...\" $styleutility />
				</center>
				";
			}
...
-->
			...
			<tr>
				<td align=\"left\"><b>Next of Kin? *</b></td>
				<td align=\"left\"><select name=\"familykin\" >
						$familyselect6a
					</select>
				</td>
			</tr>	
...
";		
				...
				while ($rowfamilylist = mysql_fetch_array($qryfamilylist))
				{
					...
					$family3 = $rowfamilylist["ADDRESS2"];
					$family3a = $rowfamilylist["ADDRESS"];
					$family3b = $rowfamilylist["MUNICIPALITY"];
					$family3c = $rowfamilylist["CITY"];
					
					if (empty($family3))
					{
						$family3 = $family3a . ", " . $family3b . " " . $family3c;
					}
...
";
				...
				while ($rowmedicallist = mysql_fetch_array($qrymedicallist))
				{
					...
					{
						// $dirfilename = $basedirdocs . $applicantno . "/D/OLD/P7_0" . $medicalcnt . ".pdf";
						$dirfilename = $basedirdocs . $applicantno . "/D/OLD/$medcode" . "_" . $add0 . $medicalcnt . ".pdf";
					...
					{
						$dirfilename = $basedirdocs . $applicantno . "/D/P7.pdf";
					}
...
echo "
					<tr $mouseovereffect>
						...
						if (checkpath($dirfilename))
							echo "
							<input type=\"button\" value=\"VIEW\" $editmode $disableapproved onclick=\"openWindow('$dirfilename', '$medicalcnt' ,700, 500);\" 
									style=\"color:Green;font-weight:Bold;font-size:0.9em;\"\" />
							";
						else 
							echo "<span style=\"font-size:1em;font-weight:Bold;color:Blue;\">--NOT SCANNED--</span>";		
...
echo "
	<div id=\"pullout\" style=\"border:1px solid Navy;display:none;$div9\">
		<span class=\"sectiontitle\">PULLOUT (NFR OR LIFT)</span>

		<br /><br />
		<table width=\"60%\" class=\"listrow\">
			<tr>
				<td align=\"left\">&nbsp;</td>
				<td align=\"left\"><b>For NFR only</b></td>
			</tr>
			<tr>
				<th align=\"left\">Remarks</th>
				<th align=\"left\">
					<textarea name=\"safekeepremarks\" onkeyup=\"javascript:this.value=this.value.toUpperCase()\"  rows=\"3\" cols=\"20\">$empreason</textarea>
				</th>
			</tr>
			<tr>
				<td align=\"left\">&nbsp;</td>
				<td align=\"left\"><input type=\"button\" value=\"Add to list...\" $disableapproved 
					onclick=\"if(safekeepremarks.value!='')
								{
									if(confirm('Are you sure this is NFR?\\nThere is no UNDO.'))
									{
										actiontxt.value='addliftwithdrawal';submit();
									}
								}else{alert('Remarks is empty!')}\" /></td>
			</tr>
";
...
";
				while ($rowpulloutlist = mysql_fetch_array($qrypulloutlist))
				{
					//SAFEKEEPREMARKS,SAFEKEEPBY,SAFEKEEPDATE,NFR,LIFTWITHDRAWAL
					$getidno = $rowpulloutlist["IDNO"];
					$pullout1 = date('m/d/Y',strtotime($rowpulloutlist["FORMDATE"]));
					if($rowpulloutlist["NFR"]==0)
						$pullout2 = $rowpulloutlist["SAFEKEEPREMARKS"];
					else 
						$pullout2="<span style=\"font-weight:bold;color:red;\">NFR</span> - ".$rowpulloutlist["SAFEKEEPREMARKS"];
					$pullout3 = $rowpulloutlist["SAFEKEEPBY"];
					if ($rowpulloutlist["SAFEKEEPDATE"] != "")
						$pullout4 = date('m/d/Y',strtotime($rowpulloutlist["SAFEKEEPDATE"]));
					else 
						$pullout4 = "";
					
					if($rowpulloutlist["LIFTWITHDRAWAL"]==0)
					{
						$pullout5="<input type=\"checkbox\" onclick=\"if(confirm('Are you sure you want this lifted?\\nThere is no UNDO.')){actiontxt.value='liftwithdrawal';editid.value='$getidno';submit();}\">";
					}
					else 
					{
						$pullout5="LIFTED";
...
echo "
					<tr $mouseovereffect>
						<td align=\"center\">&nbsp;$pullout1</td>
						<td align=\"left\">&nbsp;$pullout2</td>
						<td align=\"center\">&nbsp;$pullout3</td>
						<td align=\"center\">&nbsp;$pullout4</td>
						<td align=\"center\">&nbsp;$pullout5</td>
					</tr>
";
				}
...
echo "
	<div id=\"deceased\" style=\"border:1px solid Navy;display:none;$div10\">
		<span class=\"sectiontitle\">DECEASED</span>";
		$rowdeceasedlist=mysql_fetch_array($qrydeceasedlist);
		if(!empty($rowdeceasedlist["DATEDECEASED"]))
			$datedeceased=date("m/d/Y",strtotime($rowdeceasedlist["DATEDECEASED"]));
		$deceasedreason=$rowdeceasedlist["REASON"];
		echo "
		<br /><br />
		<table width=\"60%\" class=\"listrow\">
			<tr>
			...
			<tr>
				...
				<th align=\"left\">
					<textarea name=\"deceasedreason\" onkeyup=\"javascript:this.value=this.value.toUpperCase()\"  rows=\"3\" cols=\"20\">$deceasedreason</textarea>
				</th>
			</tr>
			<tr>
				<td align=\"left\">&nbsp;</td>
				<td align=\"left\"><input type=\"button\" value=\"Save\" 
					onclick=\"if(deceasedreason.value!='')
								{
									if(confirm('Are you sure this is correct?\\nThere is no UNDO.'))
									{
										actiontxt.value='deceased';submit();
									}
								}else{alert('Reason is empty!')}\" /></td>
			</tr>
		</table>
	</div>
";
echo "
	<div id=\"injury\" style=\"border:1px solid Navy;display:none;$div11\">
		<span class=\"sectiontitle\">INJURY</span>

		<br />
			<table style=\"width:60%;\" class=\"listrow\">
				<tr>
					<td align=\"left\"><b>Injury Date</b></td>
					<td align=\"left\">
						<input type=\"text\" name=\"injurydate\" value=\"$injurydate\" onKeyPress=\"return dateonly(this);\" size=\"10\" maxlength=\"10\" onkeydown=\"chkdatedown(this);\">
						<img src=\"calendaricon.gif\" width=\"17\" height=\"17\" border=\"0\" onclick=\"popUpCalendar(injurydate, injurydate, 'mm/dd/yyyy', 0, 0);return false;\">
						&nbsp;&nbsp;&nbsp;(mm/dd/yyyy)
					</td>
				</tr>
				<tr>
					...
					<td align=\"left\">
						<textarea rows=\"4\" cols=\"30\" name=\"injuryreason\">$injuryreason</textarea>
					</td>
				</tr>
				<tr>
					<th colspan=\"2\">Note: Fields below, SHOULD be filled up when the Crew returns from the injury.</th>
				</tr>
				<tr>
					<td align=\"left\"><b>Clinic</b></td>
					<td align=\"left\">
						<input type=\"text\" name=\"injuryclinic\" value=\"$injuryclinic\" onkeyup=\"\"  size=\"50\" />
					</td>
				</tr>
				<tr>
					<td align=\"left\"><b>Checkup Date</b></td>
					<td align=\"left\">
						<input type=\"text\" name=\"injurychkupdate\" value=\"$injurychkupdate\" onKeyPress=\"return dateonly(this);\" size=\"10\" maxlength=\"10\" onkeydown=\"chkdatedown(this);\">
						<img src=\"calendaricon.gif\" width=\"17\" height=\"17\" border=\"0\" onclick=\"popUpCalendar(injurychkupdate, injurychkupdate, 'mm/dd/yyyy', 0, 0);return false;\">
						&nbsp;&nbsp;&nbsp;(mm/dd/yyyy)
					</td>
				</tr>
				<tr>
					<td align=\"left\"><b>Diagnosis</b></td>
					<td align=\"left\">
						<textarea rows=\"4\" cols=\"30\" name=\"injurydiagnosis\">$injurydiagnosis</textarea>
					</td>
				</tr>					
				<tr>
					<td align=\"left\"><b>Recommendation</b></td>
					<th align=\"left\">
						<select name=\"injuryrecommendcode\">
							$injuryselect
						</select>
					</th>
				</tr>	
	...
	echo "
		...
		<div style=\"width:100%;overflow:auto;height:140px;background-color:#DCDCDC;\">
			<table width=\"100%\" class=\"listcol\">
				<tr>
					<th width=\"5%\">CCID</th>
					<th width=\"8%\">EMBARK</th>
					<th width=\"8%\">DISEMBARK</th>
					<th width=\"15%\">VESSEL</th>
					<th width=\"8%\">DATE</th>
					<th width=\"25%\">INJURY</th>
...
";
				...
				while ($rowexpinjurylist = mysql_fetch_array($qryexpinjurylist))
				{
					$injury1 = $rowexpinjurylist["CCID"];
					if (!empty($rowexpinjurylist["DATEEMB"]))
						$injury2 = date('m/d/Y',strtotime($rowexpinjurylist["DATEEMB"]));
					else 
						$injury2 = "";
					if (!empty($rowexpinjurylist["DATEDISEMB"]))
						$injury3 = date('m/d/Y',strtotime($rowexpinjurylist["DATEDISEMB"]));
					else 
						$injury3 = "";
					
					$injury4 = $rowexpinjurylist["VESSEL"];
					$injury4a = $rowexpinjurylist["VESSELSHORT"];
					
					if (!empty($rowexpinjurylist["DATEINJURED"]))
						$injury5 = date('m/d/Y',strtotime($rowexpinjurylist["DATEINJURED"]));
					else 
						$injury5 = "";
						
					$injury6 = $rowexpinjurylist["REASON"];
					$injury7 = $rowexpinjurylist["CLINICNAME"];
					
					if (!empty($rowexpinjurylist["DATECHECKUP"]))
						$injury8 = date('m/d/Y',strtotime($rowexpinjurylist["DATECHECKUP"]));
					else 
						$injury8 = "";
						
					$injury9 = $rowexpinjurylist["DIAGNOSIS"];
					$injury10 = $rowexpinjurylist["RECOMMENDATION"];
					
					if (!empty($injury10))
					{
						$showinjurydtls = "Clinic: " . $injury7 . "\n" .
									"Checkup Date: " . $injury8 . "\n";
									"Diagnosis: " . $injury9 . "\n";
					}
					else
					{
...
echo "
					<tr $mouseovereffect>
						<td align=\"left\">&nbsp;$injury1</td>
						<td align=\"left\">&nbsp;$injury2</td>
						<td align=\"left\">&nbsp;$injury3</td>
						<td align=\"left\" title=\"$injury4\">&nbsp;$injury4a</td>
						<td align=\"left\">&nbsp;$injury5</td>
						<td align=\"left\">&nbsp;$injury6</td>
						<td align=\"left\" title=\"$showinjurydtls\">&nbsp;$injury10</td>
						<td align=\"left\">
							<input type=\"button\" value=\"EDIT\" $editmode $disableapproved style=\"color:Red;font-weight:Bold;font-size:0.9em;\" title=\"Edit\"
									onclick=\"actiontxt.value='edit';editid.value='$injury1';submit();\" />
";	
...
echo "
	...
	<table width=\"100%\">
		<th align=\"right\">
			<input type=\"button\" name=\"btnback\" value=\"<< Back\" $disablebtnback style=\"border:2px solid gray;background-color:Red;color:Yellow;font-weight:Bold;\" onclick=\"changepage(0,currentpage.value,10);changetab();\" />
			<input type=\"button\" name=\"btnnext\" value=\"Next >>\" $disablebtnnext style=\"border:2px solid gray;background-color:Red;color:Yellow;font-weight:Bold;\" onclick=\"changepage(1,currentpage.value,10);changetab();\" />
		</th>
	</table>
