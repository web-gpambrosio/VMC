...
//		$promotedatedisemb=$rowgetlist["PROMOTEDATEDISEMB"];
	...
	//get AVAILABILTY
		...
		$qryavail=mysql_query("SELECT AVAILAILITY
									FROM debriefinghdr
									WHERE APPLICANTNO=$applicantno AND CCID=$CCID
									") or die(mysql_error());

		$rowavail=mysql_fetch_array($qryavail);

		if(empty($rowavail["AVAILABILITY"]))
			$dateavail="--";
		else
			$dateavail=date($format,strtotime($rowgetlist["AVAILABILITY"]));
		...
		<tr height=\"17px\" style=\"valign:middle;\">\n
			...
			<td style=\"$style2 text-align:center;\">&nbsp;$dateavail</td>\n
