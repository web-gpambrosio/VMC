...
if (isset($_POST['selcurrentrankcode']))
	$selcurrentrankcode = $_POST['selcurrentrankcode'];
else 
	$selcurrentrankcode = $currentrankcode;
...
if(!empty($selcurrentrankcode))
	$currentrankcode = $selcurrentrankcode;
...
<span class=\"wintitle\">TRAINING CHECKLIST BY RANK (VIEWING ONLY / NO ENROLLMENT)</span>
		...
		echo "
					...
					<center>
						<input type=\"button\" $disableprint style=\"font-size:0.9em;font-weight:Bold;border:1px solid White;color:Red;background-color:Yellow;cursor:pointer;\" 
								value=\"PRINT \n CHECKLIST\" onclick=\"openWindow('reptrainingchecklist2.php?applicantno=$applicantno&rankcode=$currentrankcode', 'repchecklist' ,0, 0);\" />
					</center>
				</div>
			...
			<span class=\"sectiontitle\">TRAINING CHECKLIST - &nbsp;$rankshow&nbsp;($rankaliasshow)
			
			<select name=\"qrankcode\" style=\"font-size:0.9em;background-color:Yellow;\" onchange=\"applicantno.value=$applicantno;selcurrentrankcode.value=this.value;submit();\">
				";
...
<!--										<td $styledtl align=\"center\">$chkrequiredshow</td>	-->
	...
	<input type=\"hidden\" name=\"selcurrentrankcode\" />
